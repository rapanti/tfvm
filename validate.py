...
parser.add_argument(
    "--crop-border-pixels", type=int, default=None, help="Crop pixels from image border."
)
parser.add_argument(
    "--mean",
    type=float,
...
def validate(args):
    ...
    if args.num_gpu > 1:
        model = torch.nn.DataParallel(model, device_ids=list(range(args.num_gpu)))  # type: ignore
    ...
    if args.real_labels:
        real_labels = RealLabelsImagenet(
            dataset.filenames(basename=True),  # type: ignore
            real_json=args.real_labels,  # type: ignore
        )
    else:
        real_labels = None
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            # compute output
            with amp_autocast():
                output = model(input)  # type: ignore

                if valid_labels is not None:
                    output = output[:, valid_labels]  # type: ignore
            ...
            acc1, acc5 = accuracy(output.detach(), target, topk=(1, 5))  # type: ignore
    ...
    results = OrderedDict(
        model=args.model,
        top1=round(top1a, 4),  # type: ignore
        top1_err=round(100 - top1a, 4),  # type: ignore
        top5=round(top5a, 4),  # type: ignore
        top5_err=round(100 - top5a, 4),  # type: ignore
